...
import { useVideoLessonStore } from '@/stores/videoLessonStore';
import {
...
export default function VideoLessons() {
  ...
  const filteredLessons = useMemo(() => {
    ...
    // Sort by order within level groups
    lessons.sort((a, b) => {
    ...
    return lessons;
  }, [store.filterLevel, store.filterTopics, store.searchQuery, store.activeLearningPathId, store.showBookmarksOnly, store.bookmarkedIds]);
  ...
  const totalLessons = VIDEO_LESSONS.length;
    ...
    <div className="stage-gradient min-h-[calc(100vh-58px)] pb-20 sm:pb-8">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p className="mt-1 font-body text-sm text-[hsl(var(--text-muted))]">
                {totalLessons} curated tutorials · {totalWatched}/{totalLessons} completed
              </p>
            </div>
              ...
              <div className="relative size-14 sm:size-16">
                <svg viewBox="0 0 36 36" className="size-full -rotate-90">
                  ...
                  <circle
                    ...
                    strokeDasharray={`${(totalWatched / totalLessons) * 97.4} 97.4`}
                    strokeLinecap="round"
                ...
                <span className="absolute inset-0 flex items-center justify-center font-display text-xs font-bold text-[hsl(var(--text-default))]">
                  {Math.round((totalWatched / totalLessons) * 100)}%
                </span>
              </div>
      ...
      <AnimatePresence>
        {activeLesson && (
          <VideoPlayerModal
            lesson={activeLesson}
            isBookmarked={store.bookmarkedIds.includes(activeLesson.id)}
            isWatched={store.watchedIds.includes(activeLesson.id)}
            onToggleBookmark={() => store.toggleBookmark(activeLesson.id)}
            onToggleWatched={() => store.toggleWatched(activeLesson.id)}
