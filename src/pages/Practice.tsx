...
export default function Practice() {
  ...
  const handleDetectionCorrect = useCallback(() => {
    ...
    if (ch) session.recordAttempt(ch.symbol, ch.name, 'correct');
  ...
  const handleNext = () => {
    if (chord) session.recordAttempt(chord.symbol, chord.name, 'skipped');
    resetBeatCounter();
    nextChord();
  };
  ...
  const handleBack = () => {
    if (session.attempts.length > 0) {
      ...
      stopListening(); stopPractice(); navigate('/chord-practice');
    }
  };
  const handleRestart = () => { resetBeatCounter(); session.startSession(); startPractice(); };
    ...
    <div className="stage-gradient min-h-[calc(100vh-58px)] flex flex-col">
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center px-3 sm:px-6 pb-[140px] sm:pb-12">
        <AnimatePresence mode="wait">
          <motion.div key={`${chord.id}-${currentIndex}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="flex flex-col items-center gap-2 sm:gap-4 w-full max-w-lg">
            <DetectionFeedback result={result} />
            <div className="text-center">
              <h2 className="font-display text-3xl sm:text-5xl md:text-6xl font-extrabold text-[hsl(var(--text-default))] leading-none">{chord.symbol}</h2>
              <p className="mt-1 sm:mt-2 font-body text-xs sm:text-base text-[hsl(var(--text-muted))]">{chord.name}</p>
            </div>
            <div className="relative min-h-[180px] sm:min-h-[220px] flex items-center justify-center mt-1 sm:mt-3 w-full">
              <AnimatePresence mode="wait">
                {showDiagrams ? (
                  <motion.div key="diagram" initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.7 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col items-center gap-3 sm:gap-6 w-full">
                    <div className="flex items-start gap-2 sm:gap-4 w-fit mx-auto max-w-[90vw] sm:max-w-none">
                      <div className="rounded-xl border border-[hsl(var(--border-default))] bg-[hsl(var(--bg-elevated)/0.8)] backdrop-blur-sm p-2 sm:p-4 glow-emphasis">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-[hsl(var(--border-default))] bg-[hsl(var(--bg-elevated)/0.95)] backdrop-blur-md safe-area-bottom">
        <div className="flex items-stretch gap-1.5 sm:gap-2 px-2 sm:px-3 py-2.5 sm:py-3 max-w-2xl mx-auto">
          <button onClick={handlePrev} className="flex items-center justify-center size-11 sm:size-12 rounded-xl border border-[hsl(var(--border-default))] bg-[hsl(var(--bg-surface))] text-[hsl(var(--text-subtle))] hover:text-[hsl(var(--text-default))] hover:bg-[hsl(var(--bg-overlay))] active:scale-95 transition-all" title="Previous">
            <SkipBack className="size-4 sm:size-5" />
          </button>
          <button onClick={handleRestart} className="flex items-center justify-center size-11 sm:size-12 rounded-xl border border-[hsl(var(--border-default))] bg-[hsl(var(--bg-surface))] text-[hsl(var(--text-subtle))] hover:text-[hsl(var(--text-default))] hover:bg-[hsl(var(--bg-overlay))] active:scale-95 transition-all" title="Restart">
            <RotateCcw className="size-4 sm:size-5" />
          </button>
          <button onClick={() => navigate('/history')} className="flex items-center justify-center size-11 sm:size-12 rounded-xl border border-[hsl(var(--border-default))] bg-[hsl(var(--bg-surface))] text-[hsl(var(--text-subtle))] hover:text-[hsl(var(--text-default))] hover:bg-[hsl(var(--bg-overlay))] active:scale-95 transition-all" title="Practice History">
            <BarChart3 className="size-4 sm:size-5" />
          </button>
          {!isRevealed ? (
            <button onClick={handleReveal} className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 rounded-xl min-h-[44px] sm:min-h-[48px] bg-[hsl(var(--color-primary)/0.15)] text-[hsl(var(--color-primary))] font-display font-bold text-xs sm:text-sm border border-[hsl(var(--color-primary)/0.3)] hover:bg-[hsl(var(--color-primary)/0.25)] active:scale-[0.97] transition-all">
              <Eye className="size-4 sm:size-5" /> <span className="hidden sm:inline">Reveal</span><span className="sm:hidden">Reveal</span>
            </button>
          ) : (
            <>
              <button onClick={() => { pauseDetection(2000); playChord(chord); }} className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 rounded-xl min-h-[44px] sm:min-h-[48px] bg-[hsl(var(--bg-surface))] text-[hsl(var(--text-subtle))] font-body font-medium text-xs sm:text-sm border border-[hsl(var(--border-default))] hover:text-[hsl(var(--text-default))] hover:bg-[hsl(var(--bg-overlay))] active:scale-[0.97] transition-all" title="Play Again">
