...
> This appendix provides precise guidance for porting the chord detection system from Web Audio API to React Native. All DSP algorithms (NSDF, chroma extraction, 6-layer voice rejection, chord matching, confusion tracking) are **pure math** and port without modification. The only changes involve the **audio input**, **spectral analysis**, and **UI/animation** layers.

---

## A.1 Library Selection

### Recommended: `react-native-live-audio-stream`

This is the most suitable library because it provides raw PCM audio data at a configurable sample rate, giving you direct access to the time-domain buffer — exactly what `useChordDetection` needs.
...
LiveAudioStream.init({
  sampleRate: 48000,       // Match web AudioContext sampleRate
  channels: 1,             // Mono (same as web)
  bitsPerSample: 16,       // 16-bit PCM
  audioSource: 6,          // Android: VOICE_RECOGNITION (bypasses AGC/NS)
  bufferSize: 4096,        // ~85ms at 48kHz — aligns with 70ms analysis cycle
});
```

### Alternative: `react-native-audio-api`

If this library is available and stable, it provides a Web Audio API-compatible interface, meaning **most of the audio pipeline code ports with minimal changes** (AudioContext, BiquadFilterNode, AnalyserNode). However, as of 2025 it may have platform gaps. If you use it, the filter chain and AnalyserNode code from Section 7 ports nearly verbatim.

### Not Recommended: `expo-av`

`expo-av` provides high-level recording APIs but does **not** expose raw PCM buffers in real-time. It is unsuitable for 14 Hz real-time chord detection.

---

## A.2 Replacing `getUserMedia` — Microphone Input
...
// → chain of BiquadFilterNodes → AnalyserNode
```
...
let pcmRingBuffer = new Float32Array(16384);
let writeIndex = 0;

LiveAudioStream.start();

LiveAudioStream.on('data', (base64: string) => {
  // Decode base64 → Int16 → Float32 (normalized to [-1, 1])
  const raw = Buffer.from(base64, 'base64');
  const samples = new Float32Array(raw.length / 2);
  for (let i = 0; i < samples.length; i++) {
    const int16 = raw.readInt16LE(i * 2);
    samples[i] = int16 / 32768.0;
  }

  // Write into ring buffer
  for (let i = 0; i < samples.length; i++) {
    pcmRingBuffer[writeIndex % pcmRingBuffer.length] = samples[i];
    writeIndex++;
  }
});
...
**Key differences:**
- No `AudioContext` — you get raw PCM directly
- No browser audio processing to disable (Android `audioSource: 6` handles this)
- Data arrives as base64 encoded Int16 — decode before processing
- Use a ring buffer to always have the latest N samples available for the analysis loop

---

## A.3 Replacing the Filter Chain — JS IIR Biquad Filters

Since there is no native `BiquadFilterNode` in `react-native-live-audio-stream`, implement the 5-stage filter chain in pure JavaScript. Each filter is a **Direct Form II Transposed** IIR biquad.
...
### Instantiate the 5-stage chain

```typescript
const SAMPLE_RATE = 48000;

const filterChain = [
  new BiquadFilter('highpass', 70, 0.71, SAMPLE_RATE),
  new BiquadFilter('notch', 50, 10, SAMPLE_RATE),
  new BiquadFilter('notch', 60, 10, SAMPLE_RATE),
  new BiquadFilter('peaking', 200, 0.5, SAMPLE_RATE, 5),
  new BiquadFilter('peaking', 500, 0.7, SAMPLE_RATE, 3),
  new BiquadFilter('lowpass', 3000, 0.5, SAMPLE_RATE),
];

function applyFilterChain(samples: Float32Array): Float32Array {
  let buf = samples;
...
## A.4 Replacing `AnalyserNode.getFloatFrequencyData` — JS FFT

The web app uses `AnalyserNode` with `fftSize: 16384` and `smoothingTimeConstant: 0.65`. In React Native, you must compute the FFT yourself.

### Recommended Library: `fft.js`

```bash
npm install fft.js
```

### FFT Implementation Matching AnalyserNode Behavior
...
// Previous frame for smoothing (matches AnalyserNode.smoothingTimeConstant)
let prevMagnitudes: Float32Array | null = null;

function getFrequencyData(timeDomainBuffer: Float32Array): Float32Array {
  // 1. Apply Hanning window
  const windowed = new Float32Array(FFT_SIZE);
  const start = Math.max(0, timeDomainBuffer.length - FFT_SIZE);
  for (let i = 0; i < FFT_SIZE; i++) {
    const idx = start + i;
    windowed[i] = idx < timeDomainBuffer.length
      ? timeDomainBuffer[idx] * hanningWindow[i]
      : 0;
  }

  // 2. Compute FFT
  const complexOutput = fft.createComplexArray();
  const realInput = fft.toComplexArray(windowed);
  fft.transform(complexOutput, realInput);

  // 3. Compute magnitude spectrum (first half only = N/2 + 1 bins)
  const numBins = FFT_SIZE / 2 + 1;
  const magnitudes = new Float32Array(numBins);
  for (let i = 0; i < numBins; i++) {
    const re = complexOutput[2 * i];
    const im = complexOutput[2 * i + 1];
    magnitudes[i] = Math.sqrt(re * re + im * im) / FFT_SIZE;
  }

  // 4. Apply temporal smoothing (matches smoothingTimeConstant behavior)
  if (prevMagnitudes) {
    for (let i = 0; i < numBins; i++) {
  ...
  // 5. Convert to dB (matches AnalyserNode.getFloatFrequencyData output)
  const freqDataDb = new Float32Array(numBins);
  for (let i = 0; i < numBins; i++) {
    freqDataDb[i] = magnitudes[i] > 0
      ? 20 * Math.log10(magnitudes[i])
      : -120; // Floor value for silence
  }
...
```
binWidth = sampleRate / FFT_SIZE = 48000 / 16384 ≈ 2.93 Hz
```

This matches the web `AnalyserNode` with the same `fftSize`.

### Bin-to-Frequency Mapping

In the web app, `extractChroma` uses `analyser.context.sampleRate / analyser.fftSize` to compute `binWidth`. In React Native, replace with:

```typescript
const binWidth = SAMPLE_RATE / FFT_SIZE; // 2.93 Hz
const freqForBin = (binIndex: number) => binIndex * binWidth;
```

---

## A.5 Porting `extractChroma` — Direct Mapping

The `extractChroma` function (Section 9) takes `freqData: Float32Array` (dB-scale frequency bins) and two scalars. Replace all `analyser` references:

| Web API Call | React Native Replacement |
|---|---|
| `analyser.context.sampleRate` | `SAMPLE_RATE` (constant: 48000) |
| `analyser.fftSize` | `FFT_SIZE` (constant: 16384) |
| `analyser.frequencyBinCount` | `FFT_SIZE / 2 + 1` (8193) |
| `analyser.getFloatFrequencyData(freqData)` | `freqData = getFrequencyData(filteredSamples)` |

The rest of `extractChroma` — spectral whitening, Gaussian interpolation, harmonic reinforcement, NSDF pitch boosting, normalization — is pure math and copies directly.

---

## A.6 Porting the Six Voice Rejection Layers

All six layers are pure JavaScript math operating on `Float32Array` inputs. Here is how each layer adapts:

### Layer 1: RMS Silence Gate
**No changes.** Operates on time-domain `Float32Array`. Use the latest 4096 samples from the ring buffer.

### Layer 2: Spectral Flatness Gate
**Signature change only.** Replace `(freqData, analyser)` with `(freqData, sampleRate, fftSize)`. All internal math (geometric mean, arithmetic mean) is pure JS.

### Layer 3: Spectral Crest Factor Gate
**Signature change only.** Same replacement: `(freqData, sampleRate, fftSize)`. Peak / mean ratio is pure math.

### Layer 4: Formant Detection Gate
**Signature change only.** Replace `analyser` parameter. The heavy moving-average smoothing, F1/F2 peak detection, prominence scoring — all pure math.

### Layer 5: Spectral Flux Gate
**No changes.** Compares current `freqData` with `prevFreqData` (stored from previous frame). Pure subtraction and half-wave rectification.

### Layer 6: Consecutive Match Thresholding
**No changes.** Integer counters comparing frame counts against thresholds. No platform dependency.

### Summary: Voice Rejection Adapter

Create a thin adapter object to replace the `analyser` parameter:

```typescript
const analyserAdapter = {
  context: { sampleRate: SAMPLE_RATE },
  fftSize: FFT_SIZE,
  frequencyBinCount: FFT_SIZE / 2 + 1,
};
```

Pass this where functions expect an `analyser` parameter, or refactor function signatures to accept `(sampleRate, fftSize)` directly.

---

## A.7 Analysis Loop — Replacing `setInterval`

### Option A: Stream-Driven (Preferred)

Process each audio buffer callback directly:

```typescript
let frameCount = 0;
const FRAMES_PER_ANALYSIS = 3; // At 48kHz with 4096 buffer ≈ 85ms per callback
                                // Analyze every ~85ms (close to web's 70ms)

LiveAudioStream.on('data', (base64: string) => {
  // Decode and push to ring buffer (see A.2)
  // ...

  frameCount++;
  if (frameCount % FRAMES_PER_ANALYSIS !== 0) return;

  // Run analysis on current ring buffer contents
  runChordAnalysis();
});
```

### Option B: Timer-Driven (Simpler, matches web pattern)

```typescript
const analysisTimer = setInterval(() => {
  // Grab latest samples from ring buffer
  const samples = getLatestSamples(FFT_SIZE);
  if (!samples) return;

  runChordAnalysis(samples);
}, 70); // Same 70ms as web
```

**Recommendation:** Option A is more efficient (no timer overhead), but Option B is a simpler 1:1 port. Both work at 14 Hz.

---

## A.8 Complete Analysis Loop — Mobile Port

```typescript
function runChordAnalysis() {
  // 1. Get latest samples from ring buffer
  const rawSamples = getLatestSamples(FFT_SIZE);
  if (!rawSamples) return;

  // 2. Apply JS filter chain (replaces Web Audio BiquadFilterNodes)
  const filtered = applyFilterChain(rawSamples);

  // 3. RMS silence gate (Layer 1) — identical to web
  const rms = computeRMS(filtered.subarray(filtered.length - 4096));
  if (rms < rmsThreshold) {
    handleSilence();
    return;
  }

  // 4. Compute FFT (replaces AnalyserNode.getFloatFrequencyData)
  const freqData = getFrequencyData(filtered);

  // 5. Voice rejection layers 2–5 (identical math, different function signatures)
  const flatness = computeSpectralFlatness(freqData, SAMPLE_RATE, FFT_SIZE);
  const crest = computeSpectralCrest(freqData, SAMPLE_RATE, FFT_SIZE);
  const formant = computeFormantScore(freqData, SAMPLE_RATE, FFT_SIZE);
  const flux = computeSpectralFlux(freqData, prevFreqData, SAMPLE_RATE, FFT_SIZE);
  prevFreqData = new Float32Array(freqData);

  const gateBlocked = (flatness > maxFlatness) || (crest < minCrest)
    || (formant > maxFormant) || (flux > 0 && flux > maxFlux);

  // 6. NSDF pitch detection (identical — operates on time-domain Float32Array)
  const nsdfPitch = autoCorrelateNSDF(filtered.subarray(filtered.length - 4096), SAMPLE_RATE, rmsThreshold);

  // 7. Chroma extraction (replace analyser references with constants)
  const chroma = extractChromaMobile(freqData, SAMPLE_RATE, FFT_SIZE, sensitivity, nsdfPitch);

  // 8. Chord matching, frame counters, result logic — identical to web
  // ...
}
```
...
## A.9 Performance Optimization for 14 Hz on Mobile

Running 6-layer voice rejection + FFT + chroma extraction + chord matching every 70ms on a mobile device requires careful optimization.

### Critical Performance Rules

1. **Pre-allocate all buffers at startup.** Never allocate `new Float32Array()` inside the analysis loop.

   ```typescript
   // Pre-allocate once
   const filteredBuffer = new Float32Array(FFT_SIZE);
   const freqDataBuffer = new Float32Array(FFT_SIZE / 2 + 1);
   const chromaBuffer = new Float64Array(12);
   const fftComplexOutput = fft.createComplexArray();
   const fftRealInput = fft.createComplexArray();
   const windowedBuffer = new Float32Array(FFT_SIZE);
   ```

2. **Reuse the FFT output array.** `fft.js` supports writing to an existing array — use `fft.transform(existingOutput, existingInput)` instead of creating new arrays.

3. **Skip unnecessary layers when possible.** If the RMS gate rejects (Layer 1), skip all subsequent layers. If spectral flatness rejects (Layer 2), skip crest/formant (Layers 3–4).

   ```typescript
   // Early exit cascade
   if (rms < rmsThreshold) { handleSilence(); return; }
   const flatness = computeSpectralFlatness(freqData, ...);
   if (flatness > maxFlatness) { handleGateBlock(); return; }
   const crest = computeSpectralCrest(freqData, ...);
   if (crest < minCrest) { handleGateBlock(); return; }
   // ... continue only if needed
   ```

4. **Avoid `Math.log10` in tight loops.** Pre-compute the dB conversion factor:

   ```typescript
   const LOG10_FACTOR = 20 / Math.LN10; // ≈ 8.6859
   const dB = LOG10_FACTOR * Math.log(magnitude); // faster than 20*Math.log10
   ```

5. **Run analysis on a background thread (optional but recommended).** Use `react-native-worklets` or `react-native-reanimated` worklets, or post raw PCM buffers to a Web Worker-like thread via `react-native-threads`. This prevents jank in the UI thread during heavy DSP.

   ```
   Audio callback → Post buffer to worker → Worker runs DSP → Post result back → UI update
   ```

6. **Reduce FFT size if needed.** If 16384-point FFT is too slow on low-end devices:
   - Try `FFT_SIZE = 8192` — doubles `binWidth` to ~5.86 Hz (still adequate for guitar)
   - Reduces FFT computation by ~50%
   - Chroma extraction accuracy decreases slightly but remains usable
   - All threshold values from Section 12 remain valid

7. **Profile with Hermes.** Ensure the Hermes JS engine is enabled (default in React Native 0.70+). Hermes compiles JS ahead-of-time and significantly improves tight-loop math performance compared to JSC.

### Performance Budget (Target: < 30ms per frame)

| Step | Estimated Time (mid-range phone) |
|---|---|
| Filter chain (6 biquads × 4096 samples) | ~1.5ms |
| FFT (16384-point via fft.js) | ~5–8ms |
| RMS computation | ~0.2ms |
| Spectral flatness | ~0.5ms |
| Spectral crest | ~0.3ms |
| Formant detection | ~1.5ms |
| Spectral flux | ~0.3ms |
| Chroma extraction | ~2–4ms |
| NSDF pitch detection | ~3–5ms |
| Chord matching | ~0.5ms |
| **Total** | **~15–22ms** |

This leaves ample headroom within the 70ms analysis window, even on mid-range devices.

---

## A.10 Animation & UI Mapping

| Web (Framer Motion) | React Native |
|---|---|
| `<motion.div animate={{ ... }}>` | `<Animated.View>` with Reanimated 3 `useAnimatedStyle` |
| `AnimatePresence` | `entering` / `exiting` layout animations in Reanimated |
| `transition={{ duration: 0.3 }}` | `withTiming(value, { duration: 300 })` |
| `initial`, `animate`, `exit` props | `FadeIn`, `FadeOut`, `SlideInDown`, `SlideOutUp` presets |
| CSS `backdrop-blur-md` | `BlurView` from `@react-native-community/blur` |
| Tailwind classes | React Native `StyleSheet` + design tokens from `designTokens.json` |

### Detection Feedback Animation
...
{result && (
  <Animated.View
    ...
    style={[styles.feedbackPill, result === 'correct' ? styles.correctPill : styles.wrongPill]}
  >
...
### Waveform Animation (Listening Indicator)

Replace the 5 `<motion.div>` bars with Reanimated `useAnimatedStyle` + `withRepeat(withTiming(...))`.

---

## A.11 State Persistence — Zustand + AsyncStorage

Replace `localStorage` persistence with `AsyncStorage`:
...
export const useDetectionSettingsStore = create<DetectionSettingsState>()(
  persist(
    (set) => ({ /* ...same store definition... */ }),
    {
      name: 'fretmaster-detection-settings',
      version: 1,
      storage: createJSONStorage(() => AsyncStorage), // ← Only change
    }
  )
);
```

Apply the same pattern to `practiceHistoryStore` (confusion matrix data) and all other persisted stores.

---

## A.12 Platform Permissions

### iOS — `Info.plist`

```xml
<key>NSMicrophoneUsageDescription</key>
<string>FretMaster needs microphone access to detect the guitar chords you play.</string>
```
  ...
  if (Platform.OS === 'android') {
    const granted = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.RECORD_AUDIO,
      {
        ...
        message: 'FretMaster needs mic access to detect chords.',
        buttonPositive: 'Allow',
      }
    );
    return granted === PermissionsAndroid.RESULTS.GRANTED;
  }
  // iOS: permission prompt is automatic on first use
  return true;
}
```

Map to `permissionDenied` state in the hook — same UX as the web version's `permissionDenied` boolean.

---

## A.13 iOS Audio Session Configuration

Critical for preventing audio conflicts and ensuring low-latency mic input:

```typescript
import { NativeModules } from 'react-native';

// In native iOS code (AppDelegate.mm or a bridged module):
...
// [session setPreferredIOBufferDuration:0.005 error:nil]; // 5ms buffer → low latency
// [session setActive:YES error:nil];
```

**Important:**
- `PlayAndRecord` category: allows simultaneous mic input + speaker output (reference tone)
- `DefaultToSpeaker`: routes audio to speaker, not earpiece
- `PreferredIOBufferDuration: 0.005`: reduces input latency to ~5ms
- Set `PreferredSampleRate: 48000` to match `react-native-live-audio-stream` config

---

## A.14 Background / App State Handling
...
useEffect(() => {
  const sub = AppState.addEventListener('change', (state) => {
    if (state === 'background' || state === 'inactive') {
      // Stop mic and analysis to save battery
      stopListening();
    }
    // Do NOT auto-resume — let user manually restart
  });
  return () => sub.remove();
}, []);
```
...
## A.15 Migration Checklist

### Audio Input
- [ ] `react-native-live-audio-stream` configured at 48kHz, mono, 16-bit
- [ ] Base64 PCM decoding implemented (Int16 → Float32 normalization)
- [ ] Ring buffer accumulates latest samples for analysis
- [ ] Android `audioSource: 6` (VOICE_RECOGNITION) disables AGC/NS

### Filter Chain
- [ ] 6 JS BiquadFilter instances match web filter chain exactly
- [ ] Highpass 70Hz, Notch 50Hz, Notch 60Hz, Peak 200Hz, Peak 500Hz, Lowpass 3000Hz
- [ ] Filters process samples before FFT

### FFT & Frequency Data
- [ ] `fft.js` with FFT_SIZE=16384 installed and working
- [ ] Hanning window pre-computed
- [ ] Temporal smoothing (0.65) matches `AnalyserNode.smoothingTimeConstant`
- [ ] Output in dB scale matches `getFloatFrequencyData` format

### Voice Rejection (all 6 layers)
- [ ] RMS silence gate — uses time-domain buffer directly
- [ ] Spectral flatness — adapted function signature, same math
- [ ] Spectral crest factor — adapted function signature, same math
- [ ] Formant detection — adapted function signature, same math
- [ ] Spectral flux — compares current vs previous frame
- [ ] Consecutive frame thresholding — integer counters, unchanged

### DSP Algorithms
- [ ] NSDF pitch detection — operates on Float32Array, no changes
- [ ] Chroma extraction — `analyser` references replaced with constants
- [ ] Chord matching (triple metric) — pure math, no changes
- [ ] Barre chord adaptation — pure math, no changes
- [ ] Confusion matrix identification — pure math, no changes

### Performance
- [ ] All Float32Array/Float64Array buffers pre-allocated
- [ ] Early exit cascade in voice rejection layers
- [ ] Total frame processing < 30ms on target device
- [ ] Hermes engine enabled
- [ ] Optional: DSP runs on background thread

### State & Persistence
- [ ] Zustand stores use `AsyncStorage` instead of `localStorage`
- [ ] Detection settings persist and sync across app sessions
- [ ] Confusion matrix data persists across sessions
- [ ] Practice history saves correctly

### UI & Animation
- [ ] Framer Motion → Reanimated 3 animations
- [ ] Detection feedback pill (Correct/Wrong) animates in/out
- [ ] Sensitivity slider works with NativeWind / StyleSheet
- [ ] Listening indicator waveform animates

### Platform
- [ ] iOS: Info.plist mic permission string set
- [ ] Android: RECORD_AUDIO permission in manifest + runtime request
- [ ] iOS: Audio session configured for PlayAndRecord, 48kHz, low latency
- [ ] App state handling: mic stops on background, no auto-resume
- [ ] `pauseDetection()` works during reference tone / chord playback
